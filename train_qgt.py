...
from quantum_models.make_model_qtemporal_gated import make_model
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP Temporal Quantum Aggregation Training")

    parser.add_argument(
        "--config_file",
        default="configs/vit_clipreid_qtemporal.yml",
    ...
    logger.info(
        f"[QGT_LR] tqa pre_net/qlayer {TQA_LR_FACTOR}× "
        f"(LR={cfg.SOLVER.STAGE2.BASE_LR * TQA_LR_FACTOR:.2e}, {n_qtg} params); "
    ...
    # ------------------------------------------------------------------ #
    # Gate analysis: log mean gate value per camera after training.
    # Runs a pass over the training loader (up to 200 batches) in eval mode,
    # collecting gate values + camera labels to show which cameras benefit most.
    # ------------------------------------------------------------------ #
    if not args.classical_ablation:
        logger.info("[Gate Analysis] Computing per-camera gate statistics...")
        model.eval()
        cam_gates = {}   # cam_id -> list of gate values
        n_gate_batches = 200
        with torch.no_grad():
            for i, batch in enumerate(train_loader_stage2):
                if i >= n_gate_batches:
                    break
                img, pid, cam, viewid, _ = batch
                img = img.cuda()
                cam = cam.cuda()
                _ = model(img, cam_label=cam, view_label=viewid.cuda())
                gates = model.qtg.last_gates   # [B]
                for c, g in zip(cam.cpu().tolist(), gates.tolist()):
                    cam_gates.setdefault(c, []).append(g)

        for cam_id in sorted(cam_gates):
            vals = cam_gates[cam_id]
            label = "AERIAL" if cam_id in (4, 5) else "ground"
            logger.info(
                f"[Gate] cam={cam_id} ({label}): "
