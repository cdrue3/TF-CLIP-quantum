...
Training script for the TF-CLIP Quantum Channel Attention variant.

Architecture: VQC generates per-channel sigmoid weights that multiplicatively
reweight CLIP features BEFORE the primary bottleneck BN:

    img_feature (768) → QuantumChannelAttention → BN → nn.Linear → cls_score
...
This is different from:
  ...
  train_qchannel.py   — VQC generates MULTIPLICATIVE channel attention weights

Usage:
    python train_qchannel.py \\
...
# Quantum channel attention model
from quantum_models.make_model_qchannel import make_model
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP Quantum Channel Attention Training")
    ...
    logger.info(
        f"[QuantumChannelAttn] n_qubits={args.n_qubits}, n_layers={args.n_layers}, "
        f"mode={mode_str}, pattern=multiplicative channel attention (before BN), "
        f"768→{args.n_qubits}→{n_q_feat}→768 weights, "
    ...
    for pg in optimizer_2stage.param_groups:
        for p in pg["params"]:
            ...
            if name.startswith("quantum_channel_attn"):
                if "expand" not in name:   # pre_net + qlayer get boost; expand standard
    ...
    logger.info(
        f"[QuantumChannelAttnLR] attn pre_net/qlayer {ADAPTER_LR_FACTOR}× "
