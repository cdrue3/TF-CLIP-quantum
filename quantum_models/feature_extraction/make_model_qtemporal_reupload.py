...
TF-CLIP model variant — Temporal Quantum Aggregation (ReuploadTQA).

What changed vs. make_model_adapter.py
---------------------------------------
- QuantumTemporalReupload replaces the mean-pool temporal aggregation in the primary path:
      img_feature.view(B, T, 768).mean(1)   →   self.tqa(img_feature.view(B, T, 768))
  ReuploadTQA encodes T frames sequentially into a shared VQC (data re-uploading over time),
...
- The later quantum_adapter application block from make_model_adapter.py is removed;
  ReuploadTQA is already applied at the pooling step, so feat = self.bottleneck(img_feature).
...
Why this differs from the adapter approach:
  ...
  ReuploadTQA (this file): [B, T, 768] → VQC sequential upload → [B, 768] → BN → nn.Linear
...
from quantum_models.feature_extraction.quantum_temporal_reupload import QuantumTemporalReupload
...
class build_transformer(nn.Module):
    ...
    Differences from make_model_adapter.py:
        - tqa: QuantumTemporalReupload — replaces img_feature.mean(1) in the primary path.
          Input [B, T, 768] → VQC sequential upload → skip → [B, 768].
          bypass_quantum=True: plain mean(1) (classical ablation).
        - No separate quantum_adapter step after pooling (ReuploadTQA is the pooling step).
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
                 n_qubits: int = 8, n_layers: int = 2, n_reupload: int = 4,
        ...
        print(
            f"[ReuploadTQA] n_qubits={n_qubits}, n_layers={n_layers}, seq_len={cfg.INPUT.SEQ_LEN}  "
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.model_name == 'RN50':
            ...
            # ReuploadTQA replaces .mean(1); bypass_quantum=True gives plain mean-pool.
        ...
        # ReuploadTQA is already applied above; just pass img_feature through BN.
        # ------------------------------------------------------------------ #
...
def make_model(cfg, num_class, camera_num, view_num,
    ...
    Args:
        ...
        n_qubits       : Qubit count for ReuploadTQA. Default 8.
        n_layers       : VQC depth. Default 2.
        bypass_quantum : If True, ReuploadTQA returns plain mean-pool (classical ablation).

    Returns:
        build_transformer instance with QuantumTemporalReupload + classical classifier heads.
    """
