...
---------------------------------------
- QuantumAutoEncoder replaces the mean-pool temporal aggregation in the primary path:
...
# Quantum autoencoder feature extractor
from quantum_models.feature_extraction.quantum_autoencoder import QuantumAutoEncoder
...
class build_transformer(nn.Module):
    ...
    Differences from make_model_adapter.py:
        - tqa: QuantumAutoEncoder — replaces img_feature.mean(1) in the primary path.
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
        ...
        # ------------------------------------------------------------------ #
        # Quantum AutoEncoder  (NEW)
        # Applied after mean-pool: [B, 768] → VQC bottleneck → [B, 768]
        # Returns (output, x_recon) during training for reconstruction loss.
        # bypass_quantum=True: passthrough (no VQC, no reconstruction loss)
        # ------------------------------------------------------------------ #
        self.tqa = QuantumAutoEncoder(
        ...
        # ------------------------------------------------------------------ #
        self._x_recon = None  # set during forward; used by train script for recon loss
        ...
        print(
            f"[QAutoEncoder] n_qubits={n_qubits}, n_layers={n_layers}  "
            f"(VQC bottleneck on mean-pooled features; reconstruction loss during training)"
        )
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.model_name == 'RN50':
            ...
            # ── QUANTUM AUTOENCODER on pooled features ───────────────────── #
            # Mean-pool first, then autoencoder processes the tracklet descriptor.
            img_feature_pooled = img_feature.view(B, T, -1).mean(1)       # [B, 768]
            img_feature, self._x_recon = self.tqa(img_feature_pooled)     # [B, 768], [B, 768]
...
def make_model(cfg, num_class, camera_num, view_num,
    ...
    Returns:
        build_transformer instance with QuantumAutoEncoder + classical classifier heads.
    """
