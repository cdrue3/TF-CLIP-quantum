...
TF-CLIP model variant — Temporal Quantum Aggregation (DeepTQA).

What changed vs. make_model_adapter.py
---------------------------------------
- QuantumTemporalDeep replaces the mean-pool temporal aggregation in the primary path:
      img_feature.view(B, T, 768).mean(1)   →   self.tqa(img_feature.view(B, T, 768))
  DeepTQA encodes T frames sequentially into a shared VQC (data re-uploading over time),
...
- The later quantum_adapter application block from make_model_adapter.py is removed;
  DeepTQA is already applied at the pooling step, so feat = self.bottleneck(img_feature).
...
Why this differs from the adapter approach:
  ...
  DeepTQA (this file): [B, T, 768] → VQC sequential upload → [B, 768] → BN → nn.Linear
...
from quantum_models.feature_extraction.quantum_temporal_parallel import QuantumTemporalParallel as QuantumTemporalDeep
...
class build_transformer(nn.Module):
    ...
    Differences from make_model_adapter.py:
        - tqa: QuantumTemporalDeep — replaces img_feature.mean(1) in the primary path.
          Input [B, T, 768] → VQC sequential upload → skip → [B, 768].
          bypass_quantum=True: plain mean(1) (classical ablation).
        - No separate quantum_adapter step after pooling (DeepTQA is the pooling step).
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
                 ...
                 dense_encoding: bool = False, hamiltonian: bool = False,
        ...
        print(
            f"[DeepTQA] n_qubits={n_qubits}, n_layers={n_layers}, seq_len={cfg.INPUT.SEQ_LEN}  "
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.model_name == 'RN50':
            ...
            # DeepTQA replaces .mean(1); bypass_quantum=True gives plain mean-pool.
        ...
        # DeepTQA is already applied above; just pass img_feature through BN.
        # ------------------------------------------------------------------ #
...
def make_model(cfg, num_class, camera_num, view_num,
               ...
               bypass_quantum: bool = False, dense_encoding: bool = False,
               hamiltonian: bool = False, fusion_mode: str = 'concat'):
    model = build_transformer(
        num_class, camera_num, view_num, cfg,
        n_qubits=n_qubits, n_layers=n_layers,
        bypass_quantum=bypass_quantum, dense_encoding=dense_encoding,
        hamiltonian=hamiltonian, fusion_mode=fusion_mode,
    )
    return model
