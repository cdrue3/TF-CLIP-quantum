"""
quantum_models/make_model_qinterlaced.py

TF-CLIP model variant — Q-C-Q Interlaced Adapter (two-stage quantum with classical bridge).

What changed vs. make_model_adapter.py
---------------------------------------
- QuantumAdapter (single VQC) → QuantumInterlacedAdapter (Q-C-Q two-VQC pipeline).
- Architecture:
    x → VQC1 → upscale1(→256) → ReLU → classical(256→256) → VQC2 → upscale2(→768) → x + delta
- All 4 classifier heads remain standard nn.Linear (no VQC in classification).

Survey paper: interlaced pattern (Q-C-Q) reported +5.7% over classical for brain tumor dataset.
Two VQC stages avoid the single-pass bottleneck; classical bridge distils intermediate features.
...
class build_transformer(nn.Module):
    """
    TF-CLIP build_transformer with Q-C-Q interlaced adapter on the primary feature path.

    Differences from make_model_adapter.py:
        - interlaced_adapter: QuantumInterlacedAdapter(768) — two VQC stages with
          classical bridge; residual delta added to img_feature before BN.
    ...
    Forward path (primary, classifier2):
        img_feature (768) → interlaced_adapter (Q-C-Q residual) → bottleneck (BN) →
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
        ...
        # x_out = x + VQC2(classical(VQC1(x)))  — two-stage residual
        # ------------------------------------------------------------------ #
        self.interlaced_adapter = QuantumInterlacedAdapter(
        ...
        print(
            f"[QuantumInterlaced] n_qubits={n_qubits}, n_layers={n_layers}  "
            f"(Q-C-Q residual on img_feature before BN; all 4 heads: classical nn.Linear)"
        )
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.training or self.neck_feat == 'after':
            img_feature_adapted = self.interlaced_adapter(img_feature)   # [B, 768], residual
            feat = self.bottleneck(img_feature_adapted)
        else:
            # neck_feat='before' eval: interlaced output is never used; skip VQC.
...
def make_model(cfg, num_class, camera_num, view_num,
    ...
    """
    Build the TF-CLIP model with Q-C-Q interlaced adapter on the primary feature path.
    ...
    Args:
        ...
        n_qubits       : Qubit count for both VQCs. Default 8.
        n_layers       : Variational layer depth. Default 2.
        bypass_quantum : If True, replace both VQCs with classical Linear+ReLU.

    Returns:
        build_transformer instance with QuantumInterlacedAdapter + classical classifier heads.
    """
