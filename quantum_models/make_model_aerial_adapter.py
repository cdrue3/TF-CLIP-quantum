"""
quantum_models/make_model_aerial_adapter.py

TF-CLIP model variant — Aerial-Selective Quantum Adapter (ASQA).

Hard binary mask applies VQC correction only to aerial-camera tracklets (C4, C5).

    aerial_mask  = 1.0 if cam ∈ {4, 5} else 0.0   — deterministic, not learned
    delta        = upscale(VQC(pre_net(x)))
    output       = x + aerial_mask * delta

Aerial cameras (70–120m altitude) produce degraded CLIP features. Literature shows
VQC advantage on noisy/constrained-dimensionality inputs. Ground cameras produce
high-quality features where no correction is applied.

What changed vs. make_model_gated_ccg.py
---------------------------------------
- Uses GatedQuantumAdapter instead of QuantumAdapter.
- gate_net: Linear(768, 1) — additional 769 parameters vs. QuantumAdapter.
- Adapter reference in model is still named quantum_adapter (LR boost compatibility).

- All 4 classifier heads remain standard nn.Linear (no VQC in classification).
  Quantum module is in the FEATURE EXTRACTION stage, not the classification stage.

- Only the primary backbone path (→ bottleneck → classifier2) is adapted.
  The three remaining heads (img_feature_proj / cls_f_sp / cls_f_tp) are
  unmodified — they retain original TF-CLIP behaviour.

Why this differs from qclassifier and qfeatext:
  qclassifier (sequential): x → [pre_net → VQC → probs] → post_net → 625
      VQC replaces the head; classical competes with quantum.
  qfeatext (parallel): [x || VQC(x)] → post_net → 625
      VQC augments the head; both classical and quantum feed the same linear layer.
  adapter (this file): img_feature → VQC_residual → BN → nn.Linear → 625
      VQC adapts features before BN; all heads remain classical.
      ONE VQC run per forward pass; shares parameters across training.

make_model() has the same signature as make_model_qclassifier.py except
bypass_quantum and encoding are removed (not applicable to the adapter pattern).
...
# ============================================================================
# Main model  — QUANTUM ADAPTER VARIANT
# ============================================================================

class build_transformer(nn.Module):
    """
    TF-CLIP build_transformer with a shared VQC adapter on the primary feature path.

    Differences from make_model_qclassifier.py:
        - quantum_adapter: QuantumAdapter(768) — residual VQC applied to img_feature
          BEFORE the primary bottleneck BN. One VQC run per forward pass.
        - classifier2 remains nn.Linear(768, num_classes) — classical.
        - All other classifier heads are classical nn.Linear (unchanged).

    Forward path (primary, classifier2):
        img_feature (768) → quantum_adapter (residual) → bottleneck (BN) →
        classifier2 (nn.Linear) → cls_score
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
        ...
        # ------------------------------------------------------------------ #
        # Shared VQC adapter  (NEW)
        # Applied to img_feature (768-dim) BEFORE bottleneck BN.
        # Residual: img_feature_adapted = img_feature + upscale(VQC(pre_net(img_feature)))
        # ------------------------------------------------------------------ #
        self.quantum_adapter = AerialSelectiveAdapter(
            self.in_planes, n_qubits=n_qubits, n_layers=n_layers,
        ...
        print(
            f"[AerialSelectiveAdapter] n_qubits={n_qubits}, n_layers={n_layers}  "
            f"(VQC residual on aerial tracklets only C4/C5; all 4 heads: classical nn.Linear)"
        )
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        # ------------------------------------------------------------------ #
        # ------------------------------------------------------------------ #
        # Quantum adapter + bottleneck projections.
        #
        # Training: adapter always runs — gradients needed for the VQC and
        #   for the classifier heads that consume `feat`.
        #
        # Eval, neck_feat='after': adapter runs to produce adapted features
        #   for retrieval (feat = BN(adapter(img_feature))).
        #
        # Eval, neck_feat='before': the eval return uses img_feature directly
        #   (not img_feature_adapted or feat). Running the adapter here would
        #   call the VQC ~170k times over MARS (11310 tracklets × ~15 dense clips)
        #   for zero benefit — the output is discarded. Skip it.
        # ------------------------------------------------------------------ #
        if self.training or self.neck_feat == 'after':
            img_feature_adapted = self.quantum_adapter(img_feature, cam_label=cam_label)   # [B, 768]
            feat = self.bottleneck(img_feature_adapted)
        else:
            # neck_feat='before' eval: adapter output is never used; skip VQC.
            feat = self.bottleneck(img_feature)

        feat_proj       = self.bottleneck_proj(img_feature_proj)   # classical
        feat_proj_frame = self.bottleneck_proj_temp(cls_f_sp)      # classical
        feat_proj_temp  = self.bottleneck_proj_temp2(cls_f_tp)     # classical
...
def make_model(cfg, num_class, camera_num, view_num,
    ...
    """
    Build the TF-CLIP model with aerial-selective VQC adapter on the primary feature path.
    ...
    Args:
        ...
        n_qubits       : Qubit count for the VQC adapter. Default 8.
        n_layers       : Variational layer depth. Default 2.
        bypass_quantum : If True, replace VQC with classical Linear+ReLU of same dims.
                         Used to ablate whether the quantum circuit (vs selective correction
                         alone) explains the adapter's performance gains.

    Returns:
        build_transformer instance with AerialSelectiveAdapter + classical classifier heads.
    """
