"""
quantum_models/make_model_qtemporal.py

TF-CLIP model variant — Temporal Quantum Aggregation (TQA).

What changed vs. make_model_adapter.py
---------------------------------------
- QuantumTemporalAgg replaces the mean-pool temporal aggregation in the primary path:
      img_feature.view(B, T, 768).mean(1)   →   self.tqa(img_feature.view(B, T, 768))
  TQA encodes T frames sequentially into a shared VQC (data re-uploading over time),
...
- The later quantum_adapter application block from make_model_adapter.py is removed;
  TQA is already applied at the pooling step, so feat = self.bottleneck(img_feature).
...
Why this differs from the adapter approach:
  ...
  TQA (this file): [B, T, 768] → VQC sequential upload → [B, 768] → BN → nn.Linear
...
# Quantum image preprocessors
from quantum_models.preprocessing.quantum_pca_preprocess import QuantumChannelPreprocess
from quantum_models.preprocessing.quantum_pie_preprocess import QuantumSpatialFilter
...
class build_transformer(nn.Module):
    """
    TF-CLIP build_transformer with Quantum Image Preprocessing before the ViT backbone.

    Differences from make_model_adapter.py:
        - tqa: QuantumTemporalAgg — replaces img_feature.mean(1) in the primary path.
          Input [B, T, 768] → VQC sequential upload → skip → [B, 768].
          bypass_quantum=True: plain mean(1) (classical ablation).
        - No separate quantum_adapter step after pooling (TQA is the pooling step).
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
                 preprocess_mode: str = "qpca",
                 n_qubits: int = 4, n_layers: int = 1, bypass_quantum: bool = False):
        super().__init__()
        ...
        # ------------------------------------------------------------------ #
        # Quantum Image Preprocessor  (NEW)
        # Applied to raw images [B*T, 3, H, W] BEFORE the ViT backbone.
        # preprocess_mode: "qpca" = channel VQC (n_qubits=4)
        #                  "qpie" = spatial grid VQC (n_qubits=2)
        # ------------------------------------------------------------------ #
        if preprocess_mode == "qpca":
            self.q_preprocess = QuantumChannelPreprocess(
                n_channels=3, n_qubits=n_qubits, n_layers=n_layers,
                bypass_quantum=bypass_quantum,
            )
        elif preprocess_mode == "qpie":
            self.q_preprocess = QuantumSpatialFilter(
                n_channels=3, n_qubits=min(n_qubits, 2), n_layers=n_layers,
                bypass_quantum=bypass_quantum,
            )
        else:
            raise ValueError(f"preprocess_mode must be qpca or qpie, got {preprocess_mode}")
        ...
        print(
            f"[QPreprocess] mode={preprocess_mode}, n_qubits={n_qubits}, n_layers={n_layers}  "
            f"(applied to raw images before ViT; mean-pool and classifiers unchanged)"
        )
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.model_name == 'RN50':
            ...
            x = self.q_preprocess(x)  # [B*T, 3, H, W] → quantum preprocessed
            image_features, image_features_proj_raw = self.image_encoder(x, cv_embed)

            img_feature      = image_features[:, 0]
            img_feature_proj = image_features_proj_raw[:, 0]

            # ── QUANTUM PREPROCESSING applied above before image_encoder ── #
            # img_feature is from ViT after quantum-preprocessed input
            img_feature      = img_feature.view(B, T, -1).mean(1)        # [B, 768] plain mean-pool
        ...
        # TQA is already applied above; just pass img_feature through BN.
        # ------------------------------------------------------------------ #
...
def make_model(cfg, num_class, camera_num, view_num,
    ...
    Args:
        ...
        n_qubits       : Qubit count for TQA. Default 8.
        n_layers       : VQC depth. Default 2.
        bypass_quantum : If True, TQA returns plain mean-pool (classical ablation).

    Returns:
        build_transformer instance with QuantumTemporalAgg + classical classifier heads.
    """
