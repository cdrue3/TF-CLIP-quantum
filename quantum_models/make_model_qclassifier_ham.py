"""
quantum_models/make_model_qclassifier.py
...
- The four nn.Linear classifier heads are replaced with QuantumClassifier
  (a hybrid VQC layer from quantum_layers.py):
      ...
      self.classifier_proj    = nn.Linear(512, num_classes)   -->  QuantumClassifier(...)
...
# Quantum classifier layer — Hamiltonian encoding
from quantum_models.amplitude.quantum_classifier_ham import QuantumClassifierHam as QuantumClassifier
...
class build_transformer(nn.Module):
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
        ...
        # ------------------------------------------------------------------ #
        # Quantum classifier heads  (CHANGED from nn.Linear)
        # ------------------------------------------------------------------ #
        # Each head: in_planes/in_planes_proj -> VQC(n_qubits) -> num_classes
        # (bypass_quantum=True replaces VQC with Linear+ReLU for ablation)
        self.classifier2 = QuantumClassifier(
            self.in_planes, self.num_classes, n_qubits=n_qubits, n_layers=n_layers,
            bypass_quantum=bypass_quantum, encoding=encoding,
        )
        self.classifier_proj = QuantumClassifier(
            self.in_planes_proj, self.num_classes, n_qubits=n_qubits, n_layers=n_layers,
            bypass_quantum=bypass_quantum, encoding=encoding,
        )
        self.classifier_proj_temp = QuantumClassifier(
            self.in_planes, self.num_classes, n_qubits=n_qubits, n_layers=n_layers,
            bypass_quantum=bypass_quantum, encoding=encoding,
        )
        self.classifier_proj_temp2 = QuantumClassifier(
            self.in_planes, self.num_classes, n_qubits=n_qubits, n_layers=n_layers,
            bypass_quantum=bypass_quantum, encoding=encoding,
        )
        ...
        print(
            f"[QuantumClassifier] n_qubits={n_qubits}, n_layers={n_layers}, encoding={encoding}  "
            f"(4 classifier heads replaced with VQC)"
        )
