"""
quantum_models/make_model_qframe.py

TF-CLIP model variant — Quantum Frame Attention (VQC temporal aggregation).

What changed vs. make_model_adapter.py
---------------------------------------
- Replaces the plain .mean(1) temporal aggregation with QuantumFrameAttention.
- VQC generates T soft attention weights to do a weighted sum of T frame features:
      img_feature = frame_attn([B, T, 768]) → [B, 768]   (weighted sum, not mean)
- The VQC task is minimal: just output T scalars (not 625 logits) → avoids multi-class issue.
- All 4 classifier heads remain standard nn.Linear (no VQC in classification).

Why this differs from QTemporal (TQA, train_qtemporal.py):
  TQA:       VQC *replaces* temporal aggregation and *replaces* classifier head.
  qframe:    VQC *weights* T frames, then hands the aggregated feature to classical heads.
...
class build_transformer(nn.Module):
    """
    TF-CLIP build_transformer with quantum frame attention for temporal aggregation.

    Differences from make_model_adapter.py:
        - frame_attn: QuantumFrameAttention(768) replaces .mean(1) for img_feature.
          Given [B, T, 768] frame features, outputs VQC-weighted [B, 768] aggregate.
    ...
    Forward path (primary, classifier2):
        [B*T, 768] → reshape [B, T, 768] → frame_attn → [B, 768] →
        bottleneck (BN) → classifier2 (nn.Linear) → cls_score

    Forward path (other 3 heads): unchanged from original TF-CLIP (still uses .mean(1)).
    """
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
        ...
        # ------------------------------------------------------------------ #
        # Quantum frame attention  (NEW)
        # Replaces .mean(1) for img_feature temporal aggregation.
        # Input: [B, T, 768] → Output: [B, 768] (attention-weighted sum)
        # ------------------------------------------------------------------ #
        self.frame_attn = QuantumFrameAttention(
        ...
        print(
            f"[QuantumFrameAttn] n_qubits={n_qubits}, n_layers={n_layers}  "
            f"(VQC weights T frames before temporal mean; all 4 heads: classical nn.Linear)"
        )
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.model_name == 'RN50':
            ...
            # Frame attention: VQC weights T frames (only during training/after-BN eval).
            # At eval with neck_feat='before', img_feature is returned directly
            # so we skip the VQC to avoid O(T*num_tracklets) VQC calls for no benefit.
            img_feature_frames = img_feature.view(B, T, -1)           # [B, T, 768]
            if self.training or self.neck_feat == 'after':
                img_feature = self.frame_attn(img_feature_frames)     # [B, 768] weighted
            else:
                img_feature = img_feature_frames.mean(1)              # [B, 768] plain mean
            img_feature_proj = img_feature_proj.view(B, T, -1).mean(1)
        ...
        # Frame attention already applied above; img_feature is the weighted aggregate.
...
def make_model(cfg, num_class, camera_num, view_num,
    ...
    """
    Build the TF-CLIP model with quantum frame attention for temporal aggregation.
    ...
    Returns:
        build_transformer instance with QuantumFrameAttention + classical classifier heads.
    """
