"""
quantum_models/make_model_gated_ccg.py

TF-CLIP model variant — Camera-Conditioned Gated Quantum Adapter (CCG).

Extends make_model_gated.py: the routing gate is conditioned on camera ID.

    cam_e  = cam_gate_embed(cam_label)          [B, 16]
    g(x)   = sigmoid(gate_net(concat(x, cam_e)))
    output = x + g * upscale(VQC(pre_net(x)))

The explicit camera embedding (16-dim) gives the gate a direct signal to route
differently for aerial (cam=1) vs ground (cam=0) tracklets in AG-ReID.
...
class build_transformer(nn.Module):
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
        ...
        # ------------------------------------------------------------------ #
        self.quantum_adapter = CameraConditionedGatedAdapter(
            self.in_planes, n_qubits=n_qubits, n_layers=n_layers,
            bypass_quantum=bypass_quantum, camera_num=camera_num,
        )
        ...
        print(
            f"[CameraConditionedGatedAdapter] n_qubits={n_qubits}, n_layers={n_layers}  "
            f"(residual on img_feature before BN; all 4 heads: classical nn.Linear)"
        )
...
def make_model(cfg, num_class, camera_num, view_num,
    ...
    """
    Build the TF-CLIP model with a shared VQC adapter on the primary feature path.
    ...
    Args:
        ...
        bypass_quantum : If True, replace VQC with classical Linear+ReLU of same dims.
                         Used to ablate whether the quantum circuit (vs residual
                         architecture alone) explains the adapter's performance gains.

    Returns:
        build_transformer instance with QuantumAdapter + classical classifier heads.
    """
