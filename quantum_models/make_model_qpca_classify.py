"""
quantum_models/make_model_qpca_classify.py

TF-CLIP model variant — Quantum Classifier integration.

What changed vs. model/make_model_clipreid.py
----------------------------------------------
- The four nn.Linear classifier heads are replaced with QuantumClassifier
  (a hybrid VQC layer from quantum_layers.py):

      Original                              This file
      --------                              ---------
      self.classifier2        = nn.Linear(768, num_classes)
      self.classifier_proj    = nn.Linear(512, num_classes)   -->  QuantumPCAClassifier(...)
      self.classifier_proj_temp  = nn.Linear(768, num_classes)
      self.classifier_proj_temp2 = nn.Linear(768, num_classes)

- make_model() accepts two new keyword arguments:
      n_qubits (int, default 8)  – qubit count / bottleneck width of VQC
      n_layers (int, default 2)  – variational entangler layer depth
...
Imports
-------
Uses absolute paths from the project root so this file can live in the
quantum_models/ subfolder without modifying any existing code.
"""

import sys
import os

# Ensure project root is on sys.path so existing model subpackages resolve.
...
# Existing TF-CLIP submodules (unchanged)
...
from quantum_models.classification.quantum_pca_classifier import QuantumPCAClassifier
...
# ============================================================================
# Main model  — QUANTUM CLASSIFIER VARIANT
# ============================================================================

class build_transformer(nn.Module):
    """
    TF-CLIP build_transformer with quantum classifier heads.

    Differences from the original:
        - classifier2, classifier_proj, classifier_proj_temp, classifier_proj_temp2
          are QuantumClassifier instances instead of nn.Linear.
        - n_qubits and n_layers control the VQC depth for all four heads.
    """

    def __init__(self, num_classes, camera_num, view_num, cfg,
                 n_qubits_pca: int = 10, n_qubits_out: int = 3,
                 n_qubits_cls: int = 8, n_layers: int = 2,
        ...
        # ------------------------------------------------------------------ #
        # Quantum PCA classifier heads  (CHANGED from nn.Linear)
        # QPCA: AmplitudeEmbedding(in→n_qubits_pca q) + SEL → partial probs
        #       → n_components=2^n_qubits_out values (quantum principal components)
        # VQC:  AngleEmbedding(n_components→n_qubits_cls q) + SEL → probs → Linear
        # ------------------------------------------------------------------ #
        self.classifier2 = QuantumPCAClassifier(
            self.in_planes, self.num_classes,
            n_qubits_pca=n_qubits_pca, n_qubits_out=n_qubits_out,
            n_qubits_cls=n_qubits_cls, n_layers=n_layers,
            bypass_quantum=bypass_quantum,
        )
        self.classifier_proj = QuantumPCAClassifier(
            self.in_planes_proj, self.num_classes,
            n_qubits_pca=n_qubits_pca, n_qubits_out=n_qubits_out,
            n_qubits_cls=n_qubits_cls, n_layers=n_layers,
            bypass_quantum=bypass_quantum,
        )
        self.classifier_proj_temp = QuantumPCAClassifier(
            self.in_planes, self.num_classes,
            n_qubits_pca=n_qubits_pca, n_qubits_out=n_qubits_out,
            n_qubits_cls=n_qubits_cls, n_layers=n_layers,
            bypass_quantum=bypass_quantum,
        )
        self.classifier_proj_temp2 = QuantumPCAClassifier(
            self.in_planes, self.num_classes,
            n_qubits_pca=n_qubits_pca, n_qubits_out=n_qubits_out,
            n_qubits_cls=n_qubits_cls, n_layers=n_layers,
            bypass_quantum=bypass_quantum,
        ...
        print(
            f"[QPCAClassifier] QPCA:{n_qubits_pca}q→{2**n_qubits_out}components, "
            f"VQC:{n_qubits_cls}q, layers={n_layers}  (4 heads replaced)"
        )
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.training:
            ...
            # -------------------------------------------------------------- #
            # Quantum classifier forward pass
            # -------------------------------------------------------------- #
...
def make_model(cfg, num_class, camera_num, view_num,
               ...
               bypass_quantum: bool = False):
    """
    Build the TF-CLIP model with quantum classifier heads.

    Args:
        cfg            : YACS config node (same as original make_model).
        num_class      : Number of identity classes in the dataset.
        camera_num     : Number of cameras.
        view_num       : Number of views.
        n_qubits       : Qubit count for each VQC head.  Default 8.
        n_layers       : Variational layer depth for each VQC head.  Default 2.
        bypass_quantum : If True, use classical Linear+ReLU instead of VQC
                         (ablation mode).  Default False.
        encoding       : Quantum encoding strategy ('angle', 'dense_angle', 'iqp').
                         Default 'angle'.

    Returns:
        build_transformer instance with QuantumClassifier heads.
    """
