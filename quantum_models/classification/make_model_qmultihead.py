...
from quantum_models.classification.quantum_multihead_classifier import QuantumMultiHeadClassifier
...
class build_transformer(nn.Module):
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
                 ...
                 n_heads: int = 4, bypass_quantum: bool = False):
        super().__init__()
        ...
        # ------------------------------------------------------------------ #
        # Multi-Head VQC Classifier  (NEW)
        # Replaces the primary classifier2 head with K independent VQC heads.
        # Mean-pool temporal aggregation is unchanged.
        # ------------------------------------------------------------------ #
        n_heads = getattr(cfg, "MODEL_MULTIHEAD_N_HEADS", 4)  # passable via cfg or hardcode
        self.classifier2 = QuantumMultiHeadClassifier(
        ...
        print(
            f"[MultiHeadVQC] n_heads={n_heads}, n_qubits={n_qubits}, n_layers={n_layers}  "
            f"(VQC replaces classifier2; other 3 heads classical; mean-pool temporal unchanged)"
        )
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.model_name == 'RN50':
            ...
            # Primary path: plain mean-pool (multi-head VQC is in the classifier)
            img_feature      = img_feature.view(B, T, -1).mean(1)        # [B, 768]
