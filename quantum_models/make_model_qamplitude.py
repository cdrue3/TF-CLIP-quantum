"""
quantum_models/make_model_qamplitude.py

TF-CLIP model variant — Temporal Quantum Aggregation (QAT).
...
- QuantumTemporalAgg replaces the mean-pool temporal aggregation in the primary path:
  ...
  QAT encodes T frames sequentially into a shared VQC (data re-uploading over time),
...
- The later quantum_adapter application block from make_model_adapter.py is removed;
  QAT is already applied at the pooling step, so feat = self.bottleneck(img_feature).
...
Why this differs from the adapter approach:
  ...
  QAT (this file): [B, T, 768] → VQC sequential upload → [B, 768] → BN → nn.Linear
...
from quantum_models.angle.quantum_amplitude_temporal import QuantumAmplitudeTemporal
...
class build_transformer(nn.Module):
    ...
    Differences from make_model_adapter.py:
        ...
        - No separate quantum_adapter step after pooling (QAT is the pooling step).
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
                 n_qubits: int = 10, n_layers: int = 2, bypass_quantum: bool = False):
        super().__init__()
        ...
        # ------------------------------------------------------------------ #
        self.qat = QuantumAmplitudeTemporal(
        ...
        print(
            f"[QAT] n_qubits={n_qubits}, n_layers={n_layers}, n_amplitudes={2**n_qubits}  "
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.model_name == 'RN50':
            ...
            # QAT replaces .mean(1); bypass_quantum=True gives plain mean-pool.
            img_feature      = self.qat(img_feature.view(B, T, -1))       # [B, 768]
        ...
        # QAT is already applied above; just pass img_feature through BN.
        # ------------------------------------------------------------------ #
...
def make_model(cfg, num_class, camera_num, view_num,
               n_qubits: int = 10, n_layers: int = 2,
    ...
    Args:
        ...
        n_qubits       : Qubit count for QAT. Default 8.
        n_layers       : VQC depth. Default 2.
        bypass_quantum : If True, QAT returns plain mean-pool (classical ablation).
