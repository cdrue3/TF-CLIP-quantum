"""
quantum_models/make_model_gated.py

TF-CLIP model variant — Input-adaptive gated quantum adapter (Research Q2).

Same as make_model_adapter.py but uses GatedQuantumAdapter:
    output = x + sigmoid(gate_net(x)) * upscale(VQC(pre_net(x)))

The gate g(x) ∈ (0,1) is learned per-sample. If g collapses to ~0 everywhere,
quantum is not useful for any input. If g varies, some inputs prefer quantum.
...
# Quantum adapter (new)
from quantum_models.angle.quantum_gated_adapter import GatedQuantumAdapter
...
class build_transformer(nn.Module):
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
        ...
        # ------------------------------------------------------------------ #
        self.quantum_adapter = GatedQuantumAdapter(
        ...
        print(
            f"[GatedQuantumAdapter] n_qubits={n_qubits}, n_layers={n_layers}  "
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.training or self.neck_feat == 'after':
            img_feature_adapted = self.quantum_adapter(img_feature)   # [B, 768], residual
