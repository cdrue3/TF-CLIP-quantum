"""
quantum_models/make_model_qchannel.py

TF-CLIP model variant — Quantum Channel Attention (multiplicative feature reweighting).

What changed vs. make_model_adapter.py
---------------------------------------
- QuantumAdapter (additive residual) → QuantumChannelAttention (multiplicative attention).
- VQC generates per-channel sigmoid weights that reweight CLIP features:
      x_out = x * sigmoid(expand(VQC(pre_net(x)))) + x
- All 4 classifier heads remain standard nn.Linear (no VQC in classification).

Why this differs from other patterns:
  adapter:  x + upscale(VQC(x))           — additive delta
  channel:  x * sigmoid(expand(VQC(x))) + x — multiplicative attention
  The multiplicative gate prevents the quantum module from adding arbitrary noise;
  it can only suppress or amplify existing features (zero init → attention ≈ 1).

Everything else (image encoder, TMD, ImageSpecificPrompt, loss interfaces,
load_param helpers) is unchanged from make_model_adapter.py.
"""
...
class build_transformer(nn.Module):
    """
    TF-CLIP build_transformer with quantum channel attention on the primary feature path.

    Differences from make_model_adapter.py:
        - quantum_channel_attn: QuantumChannelAttention(768)
          x_out = x * sigmoid(expand(VQC(pre_net(x)))) + x
          Applied to img_feature BEFORE the primary bottleneck BN.
    ...
    Forward path (primary, classifier2):
        img_feature (768) → quantum_channel_attn → bottleneck (BN) →
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg,
        ...
        # x_out = x * sigmoid(expand(VQC(pre_net(x)))) + x
        # ------------------------------------------------------------------ #
        self.quantum_channel_attn = QuantumChannelAttention(
        ...
        print(
            f"[QuantumChannelAttn] n_qubits={n_qubits}, n_layers={n_layers}  "
            f"(multiplicative attention on img_feature before BN; all 4 heads: classical nn.Linear)"
        )
    ...
    def forward(self, x=None, get_image=False, cam_label=None, view_label=None,
        ...
        if self.training or self.neck_feat == 'after':
            img_feature_adapted = self.quantum_channel_attn(img_feature)   # [B, 768]
            feat = self.bottleneck(img_feature_adapted)
        else:
            # neck_feat='before' eval: channel attn output is never used; skip VQC.
...
def make_model(cfg, num_class, camera_num, view_num,
    ...
    """
    Build the TF-CLIP model with quantum channel attention on the primary feature path.
    ...
    Args:
        ...
        n_qubits       : Qubit count for the VQC. Default 8.
        n_layers       : Variational layer depth. Default 2.
        bypass_quantum : If True, replace VQC with classical Linear+ReLU of same dims.

    Returns:
        build_transformer instance with QuantumChannelAttention + classical classifier heads.
    """
