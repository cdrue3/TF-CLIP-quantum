"""
eval_qtd_ham.py — Standalone evaluation for the QTD Hamiltonian Encoding model.
"""
...
from quantum_models.make_model_qtd_ham import make_model
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP QTD Hamiltonian Encoding — Standalone Eval")
    ...
    logger.info(f"[eval_qtd_ham] n_qubits={args.n_qubits}, n_layers={args.n_layers}")
    logger.info(f"[eval_qtd_ham] checkpoint={args.checkpoint or 'None (random weights)'}")

    val_loader, num_query, num_classes, camera_num, view_num = make_eval_all_dataloader(cfg)
    logger.info(
        f"[eval_qtd_ham] {len(val_loader)} tracklets total, "
    ...
    logger.info("[eval_qtd_ham] Building model + precomputing Pauli basis (may take ~10s)...")
    model = make_model(
        cfg,
    ...
    logger.info("[eval_qtd_ham] Model ready.")
    ...
    logger.info(f"Starting evaluation — {len(val_loader)} batches (Hamiltonian eval is slow, ~2-3x angle QTD)...")
    val_loader_pbar = tqdm(val_loader, desc="Eval", unit="batch", dynamic_ncols=True)
    r1, r5 = do_inference_dense(cfg, model, val_loader_pbar, num_query)
    logger.info(f"[eval_qtd_ham DONE] Rank-1: {r1:.1%}  Rank-5: {r5:.1%}")
