"""
eval_qchannel.py — Standalone evaluation for the Quantum Channel Attention model.
"""
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP Quantum Channel Attention — Standalone Eval")
    ...
    logger.info(f"[eval_qchannel] n_qubits={args.n_qubits}, n_layers={args.n_layers}")
    logger.info(f"[eval_qchannel] checkpoint={args.checkpoint or 'None (random weights)'}")

    val_loader, num_query, num_classes, camera_num, view_num = make_eval_all_dataloader(cfg)
    logger.info(
        f"[eval_qchannel] {len(val_loader)} tracklets total, "
    ...
    logger.info(f"[eval_qchannel DONE] Rank-1: {r1:.1%}  Rank-5: {r5:.1%}")
