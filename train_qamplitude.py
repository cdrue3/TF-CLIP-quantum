...
Architecture: QuantumAmplitudeTemporal replaces mean-pool on the primary temporal path.
...
This is different from:
  ...
  train_qamplitude.py    — VQC AGGREGATES temporal frames (replaces mean-pool)

Usage:
    python train_qamplitude.py \\
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=10,
        type=int,
        help="Number of qubits. Default 10 → 2^10=1024 amplitudes ≥ 768 (no squashing).",
    )
    parser.add_argument(
        "--n_layers",
        default=2,
    ...
    logger.info(
        f"[QAT_LR] tqa pre_net/qlayer {QAT_LR_FACTOR}× "
        f"(LR={cfg.SOLVER.STAGE2.BASE_LR * QAT_LR_FACTOR:.2e}, {n_qat} params); "
