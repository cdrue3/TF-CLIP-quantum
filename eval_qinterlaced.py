"""
eval_qinterlaced.py — Standalone evaluation for the Q-C-Q Interlaced Adapter model.
"""
...
def _pin_quantum_to_cpu(model):
    for name, module in model.named_modules():
        if hasattr(module, 'qlayer') or hasattr(module, 'qlayer1') or hasattr(module, 'qlayer2'):
            for attr in ('qlayer', 'qlayer1', 'qlayer2'):
                if hasattr(module, attr):
                    getattr(module, attr).to(device=torch.device("cpu"), dtype=torch.float32)


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description="TF-CLIP Q-C-Q Interlaced Adapter — Standalone Eval")
    parser.add_argument("--config_file", default="configs/vit_clipreid_qclassifier.yml", type=str)
    ...
    logger.info(f"[eval_qinterlaced] n_qubits={args.n_qubits}, n_layers={args.n_layers}")
    logger.info(f"[eval_qinterlaced] checkpoint={args.checkpoint or 'None (random weights)'}")

    val_loader, num_query, num_classes, camera_num, view_num = make_eval_all_dataloader(cfg)
    logger.info(
        f"[eval_qinterlaced] {len(val_loader)} tracklets total, "
    ...
    logger.info(f"[eval_qinterlaced DONE] Rank-1: {r1:.1%}  Rank-5: {r5:.1%}")
