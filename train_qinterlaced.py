"""
train_qinterlaced.py

Training script for the TF-CLIP Q-C-Q Interlaced Adapter variant.

Architecture: Two VQC stages with a classical bottleneck between them.
Survey paper's interlaced pattern — quantum→classical→quantum avoids single-pass bottleneck:

    img_feature (768) → VQC1 → upscale1(→256) → classical(256) → VQC2 → upscale2(→768) → +x
    Other 3 heads: unchanged from original TF-CLIP.

This is different from:
  train_qadapter.py     — single VQC adapter (additive residual)
  train_qinterlaced.py  — Q-C-Q two-stage residual (interlaced pattern)

Usage:
    python train_qinterlaced.py \\
...
# Q-C-Q interlaced adapter model
from quantum_models.make_model_qinterlaced import make_model
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP Q-C-Q Interlaced Adapter Training")
    ...
    logger.info(
        f"[QuantumInterlaced] n_qubits={args.n_qubits}, n_layers={args.n_layers}, "
        f"mode={mode_str}, pattern=Q-C-Q interlaced residual (before BN), "
        f"768→{args.n_qubits}→{n_q_feat}→256→{args.n_qubits}→{n_q_feat}→768, "
    ...
    for pg in optimizer_2stage.param_groups:
        for p in pg["params"]:
            ...
            if name.startswith("interlaced_adapter"):
                # pre_net1/2 and qlayer1/2 get 3× LR; upscale2 (residual output) standard
                if "upscale2" not in name:
    ...
    logger.info(
        f"[QuantumInterlacedLR] interlaced pre_nets/qlayers {ADAPTER_LR_FACTOR}× "
