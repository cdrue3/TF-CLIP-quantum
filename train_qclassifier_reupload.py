...
Training script for the TF-CLIP Quantum Temporal Difference (QTD) variant.

Architecture: QuantumTemporalAgg replaces mean-pool on the primary temporal path.
    [B, T, 768] → QTD (VQC on frame differences + skip) → [B, 768] → BN → nn.Linear
...
This is different from:
  ...
  train_qtd.py    — VQC AGGREGATES temporal frames (replaces mean-pool)
...
from quantum_models.make_model_qclassifier_reupload import make_model
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP QClassifier Re-upload Training")
    ...
    parser.add_argument(
        "--preprocess", default="none", type=str,
        choices=["none", "edge", "phase"],
        help="Quantum-inspired feature preprocessing before block re-uploading.",
    )
    parser.add_argument(
        "opts",
    ...
    logger.info(
        f"[QClassifierReupload] n_qubits={args.n_qubits}, n_layers={args.n_layers}, "
        f"seq_len={cfg.INPUT.SEQ_LEN}, mode={mode_str}, "
        f"pattern=motion diff re-uploading (residual on mean-pool), "
        f"qtd: 768→{args.n_qubits}→{n_q_feat}→768 (skip + mean-pool), "
    ...
    #
    # qtd.pre_net / qlayer: 3× — conservative, avoids sigmoid drift.
    # qtd.upscale: standard LR (1×) — takes gradient from classical heads.
    ...
    logger.info(
        f"[TQA_LR] qtd pre_net/qlayer {TQA_LR_FACTOR}× "
        f"(LR={cfg.SOLVER.STAGE2.BASE_LR * TQA_LR_FACTOR:.2e}, {n_qtd} params); "
