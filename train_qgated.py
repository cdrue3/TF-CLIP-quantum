...
Training script for the TF-CLIP Gated Quantum Adapter variant (Research Q2).

Architecture: Input-adaptive gate controls quantum correction per sample.

    g(x) = sigmoid(gate_net(x))  ∈ (0, 1)   — per-sample gate
    output = x + g * upscale(VQC(pre_net(x))) — gated residual

If g collapses to 0 everywhere: quantum is not useful for any input.
If g varies: the gate reveals which tracklets prefer quantum correction.
This directly addresses KIT Q2: "which inputs benefit from quantum?"

Usage:
    python train_qgated.py \\
...
# Gated quantum adapter model
from quantum_models.make_model_gated import make_model
