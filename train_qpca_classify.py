...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--n_qubits_pca", default=10, type=int,
        help="QPCA stage qubits. 2^n >= in_features. Default 10 → 1024 ≥ 768.",
    )
    parser.add_argument(
        "--n_qubits_out", default=3, type=int,
        help="Qubits to measure from QPCA → 2^n principal components. Default 3 → 8.",
    )
    parser.add_argument(
        "--n_qubits_cls", default=8, type=int,
        help="Classifier VQC qubits. Must equal 2^n_qubits_out. Default 8.",
    )
    parser.add_argument(
        "--n_layers",
        default=2,
        type=int,
        help="SEL depth for both QPCA and classifier circuits. Default 2.",
    )
    parser.add_argument(
        "opts",
    ...
    parser.add_argument(
        "--eval_period", default=10, type=int,
        help="Eval Rank-1 every N epochs during training. Default 10. Set to 999 to disable.",
    )
    ...
    logger.info(
        f"[QPCAClassifier] QPCA:{args.n_qubits_pca}q→{2**args.n_qubits_out}components, "
        f"VQC:{args.n_qubits_cls}q, layers={args.n_layers}, mode={mode_str}, eval_period={args.eval_period}"
    )
    ...
    # ------------------------------------------------------------------ #
    cfg.defrost()
    cfg.SOLVER.STAGE2.EVAL_PERIOD = args.eval_period
    cfg.freeze()
    ...
    # ------------------------------------------------------------------ #
    n_measurements = 2 ** args.n_qubits_cls   # probs() output dim from classifier VQC
