...
Training script for the TF-CLIP Dense-Angle Temporal Quantum Aggregation (TQA) variant.
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP Dense-Angle Temporal Quantum Aggregation Training")
    ...
    logger.info(
        f"[Dense-TQA] n_qubits={args.n_qubits}, n_layers={args.n_layers}, "
        f"seq_len={cfg.INPUT.SEQ_LEN}, mode={mode_str}, "
        f"pattern=dense angle encoding (RY+RZ, 2x features/qubit), "
        f"pre_net: 768→{2*args.n_qubits}, tqa: 768→{2*args.n_qubits}→{n_q_feat}→768 (skip + mean-pool), "
