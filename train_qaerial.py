...
Training script for the TF-CLIP Aerial-Selective Quantum Adapter (ASQA).

Architecture: Hard binary mask applies VQC correction only to aerial tracklets (C4, C5).

    aerial_mask  = 1.0 if cam ∈ {4, 5} else 0.0   — deterministic, not learned
    output = x + aerial_mask * upscale(VQC(pre_net(x)))

Ground-camera tracklets are passed through unchanged.
Hypothesis: aerial cameras (70–120m) produce degraded CLIP features where quantum
nonlinearity adds the most benefit. Classical ablation tests selective-correction alone.

Usage:
    python train_qaerial.py \\
...
# Aerial-selective quantum adapter model
from quantum_models.make_model_aerial_adapter import make_model
...
if __name__ == '__main__':
    ...
    logger.info(
        ...
        f"mode={mode_str}, pattern=residual (before BN), "
        f"adapter: 768→{args.n_qubits}→{n_q_feat}→768 (1 adapter run/forward), "
