...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP Quantum Label Refiner (QPLR) Training")
    ...
    parser.add_argument("--top_k_classes", default=32, type=int,
        help="Top-K classes passed to QPLR VQC (default 32).")
    parser.add_argument("--kl_weight", default=0.5, type=float,
        help="Weight for QPLR KL divergence loss (default 0.5).")
    parser.add_argument("--qplr_n_qubits", default=8, type=int,
        help="Qubits in QPLR VQC (default 8).")
    parser.add_argument("--qplr_n_layers", default=2, type=int,
        help="Layers in QPLR VQC (default 2).")
    parser.add_argument(
        "--classical_ablation",
        action="store_true",
        default=False,
    ...
    loss_func, center_criterion, q_refiner = make_loss_qplr(
        ...
        n_qubits=args.qplr_n_qubits,
        n_layers=args.qplr_n_layers,
    )
    model.q_refiner = q_refiner  # register as submodule for optimizer
