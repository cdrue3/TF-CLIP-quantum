"""
eval_qframe.py — Standalone evaluation for the Quantum Frame Attention model.
"""
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP Quantum Frame Attention — Standalone Eval")
    ...
    logger.info(f"[eval_qframe] n_qubits={args.n_qubits}, n_layers={args.n_layers}")
    logger.info(f"[eval_qframe] checkpoint={args.checkpoint or 'None (random weights)'}")

    val_loader, num_query, num_classes, camera_num, view_num = make_eval_all_dataloader(cfg)
    logger.info(
        f"[eval_qframe] {len(val_loader)} tracklets total, "
    ...
    logger.info(f"[eval_qframe DONE] Rank-1: {r1:.1%}  Rank-5: {r5:.1%}")
