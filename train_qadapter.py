...
Training script for the TF-CLIP Quantum Adapter variant.

Architecture: Shared VQC adapter inserted BEFORE the primary bottleneck BN.
One VQC run per forward pass; all 4 classifier heads remain classical nn.Linear.

    img_feature (768) → QuantumAdapter(residual) → BN → nn.Linear → cls_score
...
This is different from:
  ...
  train_qadapter.py    — VQC ADAPTS features BEFORE BN (residual, shared)

Usage:
    python train_qadapter.py \\
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TF-CLIP Quantum Adapter Training")
    ...
    parser.add_argument(
        ...
        help="VQC encoding: 'angle' (standard, 1 feature/qubit) or "
             "'dense_angle' (RY+RZ per qubit, 2 features/qubit). Default: angle.",
    )
    parser.add_argument(
    ...
    pre_out = 2 * args.n_qubits if args.encoding == "dense_angle" else args.n_qubits
    logger.info(
        f"[QuantumAdapter] n_qubits={args.n_qubits}, n_layers={args.n_layers}, "
        f"encoding={args.encoding}, mode={mode_str}, pattern=residual (before BN), "
        f"adapter: 768→{pre_out}→{n_q_feat}→768 (1 adapter run/forward), "
    ...
    for pg in optimizer_2stage.param_groups:
        for p in pg["params"]:
            ...
            if name.startswith("quantum_adapter"):
                if "upscale" not in name:   # pre_net + qlayer get boost; upscale standard
    ...
    logger.info(
        f"[QuantumAdapterLR] adapter pre_net/qlayer {ADAPTER_LR_FACTOR}× "
